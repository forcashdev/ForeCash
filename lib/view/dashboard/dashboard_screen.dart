...
import 'package:fore_cash/view/dashboard/widgets/monthly_income_widget.dart';
import 'package:fore_cash/view/dashboard/widgets/weekly_income_widget.dart';
...
class _DashBoardScreenState extends State<DashBoardScreen> {
  ...
  int totalAmount = 0;
  Rx<DateTime> currentDate = DateTime
      .now()
      .obs;
  ...
  List<List> tempShowMonthlyIncomeData = [];
                                                                                          ...
                                                                                          return SizedBox(
                                                                                            ...
                                                                                            child: Text(
                                                                                              ...
                                                                                              //
                                                                                                  ? '${GetIncomeController.to.monthlyIncomeList![index].amount}'
                                                                                                  : '-',
                                                                                              // tempShowMonthlyIncomeData[dateIndex][index].toString(),
                                                                                              style: greyDateTexStyle10W400,
                                                                                              textAlign: TextAlign.center,
                                                                                            ),
                                                                                          );
                                                                                          ...
                                                                                          builder: (context, snapshot) {
                                                                                            return ListView.builder(
                                                                                              ...
                                                                                              itemBuilder: (context, dateIndex) {
                                                                                                ...
                                                                                                DateTime temp = formatter.parse(TotalIncomeExpenseController.to.datesList[dateIndex]);
                                                                                                DateTime dateFormatedWeeklyDate = DateTime.parse(
  ...
  calculateFutureDates() {
    ...
    List<String> tempMonthlyIncome = [];
    for (int i = 0; i < 15; i++) {
      returnedDate = TotalIncomeExpenseController.to.findFirstDateOfTheWeek(i == 0 ? selectedResetWeekDay!=null?selectedResetWeekDay!:today : returnedDate!.add(const Duration(days: 7)));
      ...
      //     print('ppppp');
      //     for (int k = 0; k < 7; k++) {
      //       DateTime durationAddedDate = returnedDate.add(Duration(days: k));
      //       print('KKK$k');
      //       print('????$durationAddedDate');
      //     if (formatter.parse(GetIncomeController.to.monthlyIncomeList![j].date.toString()).toString().replaceRange(0, 4, '') ==
      //         formatter.parse(durationAddedDate.toString()).toString().replaceRange(0, 4, '')) {
      //       print(formatter.format(durationAddedDate).toString().replaceRange(0, 4, ''));
      //       print('********');
      //       TotalIncomeExpenseController.to.totalMonthlyIncomeList.add(GetIncomeController.to.monthlyIncomeList![j].amount.toString());
      //     } else {
      //       TotalIncomeExpenseController.to.totalMonthlyIncomeList.add('-');
      //       print('llll');
      //     }
      //   }
      // }
      // tempShowMonthlyIncomeData.add(tempMonthlyIncome);
      // print(returnedDate);
      // print(lastDateOfTheWeek);
      ...
      // tempMonthlyIncome.clear();

    }
    // print('***${TotalIncomeExpenseController.to.datesList.length}');
    // print('***${GetIncomeController.to.monthlyIncomeList!.length}');
    // for (int j = 0; j < GetIncomeController.to.monthlyIncomeList!.length; j++) {
    //   for (int k = 0; k < TotalIncomeExpenseController.to.datesList.length; k++) {
    //     print(';;;;;');
    //     if (DateFormat('dd-MM-yyyy').parse(GetIncomeController.to.monthlyIncomeList![j].date!).isBefore(DateTime.parse(TotalIncomeExpenseController.to.datesList[k]))) {
    //     // if (DateTime.parse(TotalIncomeExpenseController.to.datesList[k]).isBefore(DateFormat('yyyy-MM-dd').parse(GetIncomeController.to.monthlyIncomeList![j].date!))) {
    //       print('%%%');
    //       // tempMonthlyIncome.add(GetIncomeController.to.monthlyIncomeList?[j].amount);
    //       // tempShowMonthlyIncomeData.add(GetIncomeController.to.monthlyIncomeList?[j].amount);
    //       // print('<<<<<${GetIncomeController.to.monthlyIncomeList?[j].amount}');
    //     } else {
    //       print('****');
    //       // tempMonthlyIncome.add('-');
    //       // tempShowMonthlyIncomeData.add('-');
    //     }
    //   }
    // }

    //tempShowMonthlyIncomeData.add(tempMonthlyIncome);
    // print(tempShowMonthlyIncomeData);
    // print(';;;;;;${tempShowMonthlyIncomeData.length}');
    // print(';;;;;;${GetIncomeController.to.monthlyIncomeList!.length}');
  }
  ...
  Future<dynamic>? calculatePastDates() {
    DateTime? startDate;
    for (int i = 0; i < 15; i++) {
      startDate = TotalIncomeExpenseController.to.findFirstDateOfTheWeek(i == 0 ? selectedResetWeekDay!=null? selectedResetWeekDay!.subtract(const Duration(days: 7)):DateTime.now().subtract(const Duration(days: 7)) : startDate!.subtract(const Duration(days: 7)));
